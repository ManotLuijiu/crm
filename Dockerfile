...
# ─────────────────────────────────────────────────────────────────────
# Crm custom image for Frappe Docker
#
...
#
#   export CUSTOM_IMAGE=bunchee/crm
...
ARG APP_REPO=https://github.com/ManotLuijiu/crm

# APP_REF — which ref of this app to install
#   - "version-16"  (default; latest commits)
#   - "v1.0.0"            (pinned release tag; use for production)
ARG APP_REF=version-16
...
RUN apk add --no-cache git \
 ...
 && cd /tmp/app \
 && (git log -1 --format='crm @ %H | %s | %an <%ae>' > /tmp/app_version.txt) \
...
LABEL org.opencontainers.image.title="Crm" \
      org.opencontainers.image.description="CRM with custom fields and workflows" \
      org.opencontainers.image.source="https://github.com/ManotLuijiu/crm" \
...
COPY --chown=frappe:frappe --from=fetcher /tmp/app /home/frappe/frappe-bench/apps/crm
COPY --chown=frappe:frappe --from=fetcher /tmp/app_version.txt /home/frappe/frappe-bench/apps/crm_version.txt
...
RUN bench setup requirements \
 ...
 && (python -c "import crm; print('Crm:', getattr(crm, '__version__', 'unknown'))" || echo "  (skip per-app import — module name may differ)") \
 && cat /home/frappe/frappe-bench/apps/crm_version.txt \
 && echo "✅ Crm image built from ${APP_REF}"
